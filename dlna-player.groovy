...
metadata {
	...
	definition (name: "DLNA Player", namespace: "com.openmobl.device.dlna", author: "SmartThings") {
		capability "Actuator"
		capability "Switch"
		capability "Refresh"
		capability "Sensor"
...
def setLocalLevel(val, delay=0) {
	...
	result << dlnaAction("SetVolume", "RenderingControl", getRenderingControlUrl(), [InstanceID: 0, Channel: "Master", DesiredVolume: v])
	//result << delayAction(500),
	result << dlnaAction("GetVolume", "RenderingControl", getRenderingControlUrl(), [InstanceID: 0, Channel: "Master"])
	result
}
...
def play() {
	log.trace "play()"
	coordinate({dlnaAction("Play")}, {it.play()})
}

def stop() {
	log.trace "stop()"
	coordinate({dlnaAction("Stop")}, {it.stop()})
}

def pause() {
	log.trace "pause()"
	coordinate({dlnaAction("Pause")}, {it.pause()})
}

def nextTrack() {
	log.trace "nextTrack()"
	coordinate({dlnaAction("Next")}, {it.nextTrack()})
}

def previousTrack() {
	log.trace "previousTrack()"
	coordinate({dlnaAction("Previous")}, {it.previousTrack()})
}

def seek(trackNumber) {
	log.trace "seek($trackNumber)"
	coordinate({dlnaAction("Seek", "AVTransport", getControlUrl(), [InstanceID: 0, Unit: "TRACK_NR", Target: trackNumber])}, {it.seek(trackNumber)})
}
...
def mute()
{
	...
	dlnaAction("SetMute", "RenderingControl", getRenderingControlUrl(), [InstanceID: 0, Channel: "Master", DesiredMute: 1])
}
...
def unmute()
{
	...
	dlnaAction("SetMute", "RenderingControl", getRenderingControlUrl(), [InstanceID: 0, Channel: "Master", DesiredMute: 0])
}

def setPlayMode(mode)
{
	log.trace "setPlayMode($mode)"
	coordinate({dlnaAction("SetPlayMode", [InstanceID: 0, NewPlayMode: mode])}, {it.setPlayMode(mode)})
}
...
def setTrack(Map trackData) {
	...
	coordinate({
		...
		if ((data.transportUri.startsWith("x-rincon-queue:") || data.enqueuedUri.contains("savedqueues")) && data.trackNumber != null) {
			...
			result << dlnaAction("RemoveAllTracksFromQueue", [InstanceID: 0])
			//result << delayAction(500)
			result << dlnaAction("AddURIToQueue", [InstanceID: 0, EnqueuedURI: data.uri, EnqueuedURIMetaData: data.metaData, DesiredFirstTrackNumberEnqueued: 0, EnqueueAsNext: 1])
			//result << delayAction(500)
			result << dlnaAction("SetAVTransportURI", [InstanceID: 0, CurrentURI: uri, CurrentURIMetaData: metaData])
			//result << delayAction(500)
			result << dlnaAction("Seek", "AVTransport", getControlUrl(), [InstanceID: 0, Unit: "TRACK_NR", Target: data.trackNumber])
		} else {
	...
	coordinate({
		...
		if (data) {
			if ((data.transportUri.startsWith("x-rincon-queue:") || data.enqueuedUri.contains("savedqueues")) && data.trackNumber != null) {
				...
				result << dlnaAction("SetAVTransportURI", [InstanceID: 0, CurrentURI: uri, CurrentURIMetaData: data.metaData])
				//result << delayAction(500)
				result << dlnaAction("Seek", "AVTransport", getControlUrl(), [InstanceID: 0, Unit: "TRACK_NR", Target: data.trackNumber])
			} else {
				log.trace "Setting track to $data.uri"
				//setTrack(data.uri, null, data.metaData)
				result << dlnaAction("SetAVTransportURI", [InstanceID: 0, CurrentURI: data.uri, CurrentURIMetaData: data.metaData])
			}
		}
		else {
...
def subscribe() {
	log.trace "subscribe()"
	def result = []
	result << subscribeAction(getAvTransEventUrl())
	result << delayAction(10000)
	result << subscribeAction(getRenderingControlEventUrl())
...
def unsubscribe() {
	...
	def result = [
		unsubscribeAction(getAvTransEventUrl(), device.getDataValue('subscriptionId')),
		//delayAction(500),
		unsubscribeAction(getRenderingControlEventUrl(), device.getDataValue('subscriptionId')),
		//delayAction(500),

		unsubscribeAction(getAvTransEventUrl(), device.getDataValue('subscriptionId1')),
		//delayAction(500),
		unsubscribeAction(getRenderingControlEventUrl(), device.getDataValue('subscriptionId1')),
		//delayAction(500),

		unsubscribeAction(getAvTransEventUrl(), device.getDataValue('subscriptionId2')),
		//delayAction(500),
		unsubscribeAction(getRenderingControlEventUrl(), device.getDataValue('subscriptionId2')),
		//delayAction(500),
	]
...
{
	log.trace "getVolume()"
	dlnaAction("GetVolume", "RenderingControl", getRenderingControlUrl(), [InstanceID: 0, Channel: "Master"])
}

def getCurrentMedia()
{
	log.trace "getCurrentMedia()"
	dlnaAction("GetPositionInfo", [InstanceID:0, Channel: "Master"])
}

def getCurrentStatus() //transport info
{
	log.trace "getCurrentStatus()"
	dlnaAction("GetTransportInfo", [InstanceID:0])
}

def getSystemString()
{
	log.trace "getSystemString()"
	dlnaAction("GetString", "SystemProperties", "/SystemProperties/Control", [VariableName: "UMTracking"])
}
...
private dlnaAction(String action) {
	dlnaAction(action, "AVTransport", getControlUrl(), [InstanceID:0, Speed:1])
}

private dlnaAction(String action, Map body) {
	dlnaAction(action, "AVTransport", getControlUrl(), body)
}

private dlnaAction(String action, String service, String path, Map body = [InstanceID:0, Speed:1]) {
	log.trace "dlnaAction($action, $service, $path, $body)"
	def result = new physicalgraph.device.HubSoapAction(
		path:    path ?: getControlUrl(),
...
private getDeviceDefines() {
	return parent.getSpecificDlnaPlayer(this)
}

private getControlUrl() {
	return getDeviceDefines().value.controlUrl
}

private getAvTransEventUrl() {
	return getDeviceDefines().value.avTransEventUrl
}

private getRenderingControlUrl() {
	return getDeviceDefines().value.renderingControlUrl
}

private getRenderingControlEventUrl() {
	return getDeviceDefines().value.renderingControlEventUrl
}
